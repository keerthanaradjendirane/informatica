...
if menu == "Upload Data":
    ...
    data_source = st.radio("Select Data Source", ["CSV/JSON File", "MongoDB Database","Informatica"])

    # Automatically load default CSV file without replacing uploads
    default_csv_path = r"C:\Users\keert\Downloads\heart_disease.csv"
    
    if "data" not in st.session_state:  # Only load if no other data is present
        try:
            data = pd.read_csv(default_csv_path)
            st.session_state['data'] = data
            st.success(f"Automatically loaded: {default_csv_path}")
            st.write("### Default Dataset Preview:")
            st.dataframe(data)
        except Exception as e:
            st.warning(f"Could not load default CSV: {e}")

    # Handling file upload (Existing functionality)
    ...
    # Handling MongoDB fetch (Existing functionality)
        ...
        if st.button("Fetch Data from MongoDB"):
            try:
                ...
                if documents:
                    ...
                    st.write("### MongoDB Dataset Preview:")
                    st.dataframe(data)
                else:
                    st.warning("No data found in the collection.")
            except Exception as e:
                st.error(f"Error connecting to MongoDB: {e}")
    elif data_source == "Informatica":
        st.subheader("Connect to Informatica Database")
        db_category = st.selectbox("Select Database Category", ["Relational", "Cloud", "NoSQL"])
        ...
        db_type = st.selectbox("Select Database Type", db_options[db_category])
        st.text_input("Host / Server", "your-database-host.com")
        st.text_input("Port", "3306")
        st.text_input("Database Name", "your_database")
        st.text_input("Username", "your_username")
        st.text_input("Password", type="password")

        if st.button("Fetch Data"):
            file_path = r"C:\\Users\\keert\\Downloads\\heart_disease.csv"  # Example CSV file
            try:
                df = pd.read_csv(file_path)
                st.write("### Sample Data from Informatica Connection:")
                st.dataframe(df)
            except Exception as e:
